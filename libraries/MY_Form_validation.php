...
class MY_Form_validation extends CI_Form_validation {
    ...
    public function set_rules($field, $label = '', $rules = array(), $errors = array()) {
        $method = (method_exists($this->CI->input, 'method')) ?
            $this->CI->input->method() :
            $this->CI->input->server('REQUEST_METHOD');

        $this->request_method = (!empty($method)) ? strtolower($method) : 'post';

        // No reason to set rules if we have no POST data
        // or a validation array has not been specified
        if ($this->request_method !== 'post' && empty($this->validation_data)) {
            return $this;
        }
